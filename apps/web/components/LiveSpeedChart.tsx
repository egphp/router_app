...
export function LiveSpeedChart() {
  const { data } = useSWR<{ speeds: SpeedPoint[] }>('/api/live?minutes=60', fetcher, { refreshInterval: 5000 });
  const rows = useMemo(() => {
    return (data?.speeds ?? []).map((p) => ({
    ...
    <div className="card p-5 animate-fade-in">
      <div className="flex items-center justify-between mb-4">
        <div>
          ...
          <div className="text-xs text-slate-500 mt-1">Last 60 minutes · all devices</div>
        </div>
      ...
      <div className="h-64">
        <ResponsiveContainer width="100%" height="100%">
          <AreaChart data={rows} margin={{ top: 8, right: 8, bottom: 0, left: 8 }}>
            ...
            <XAxis dataKey="t" stroke="#64748b" tick={{ fontSize: 11 }} minTickGap={48} />
